...
.NOTES
  ...
  Purpose/Change: Added default parameters, cleanup and a cool process-block
...
Param(
    ...
    [string]$MeasureServer = "www.sunet.se",
    ...
    [Parameter()]
    [ValidateNotNullOrEmpty()]
    [string]$TemporaryDownloadPath = "$env:TEMP\DownloadUpdates",
...
begin {
    ...
    function Test-InternetConnection {
        ...
        if ($MeasureServer.Length -eq 0) {
            Write-Host "Hmm, the measure server variable is empty. We really need something to ping :)"
            break
        }
        else {
    ...
    function Test-SharedFolder {
        ...
        Catch {
            Write-Host "No, we couldn't. Lovely! Exiting and slowly crawling my way back to the coffee machine."
        }
    ...
    function Test-RebootRequired {
        ...
        if ($result.ContainsValue($true)) {
            ...
            Write-Host "No need for a reboot right now..move a lalala long"
        }

        return $result.ContainsValue($true)
    }

    # Logic from Client Health Script
    # https://gallery.technet.microsoft.com/ConfigMgr-Client-Health-ccd00bd7
    ...
    If (Test-RebootRequired) { Start-RebootTask; $ShouldContinue = $false }
    if (!(Test-InternetConnection)) { Write-Host "The connection is not good enough for downloading updates. Trying later..."; $ShouldContinue = $false }
    if (!(Test-SharedFolder)) { $ShouldContinue = $false }
}
process {
